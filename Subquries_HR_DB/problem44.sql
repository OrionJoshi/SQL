...
-- Method-1
    ...
    FROM employees WHERE salary < (SELECT MIN(salary) FROM employees WHERE department_id = 70)

-- Method-2

    SELECT first_name, last_name, salary, department_id 
    FROM employees WHERE salary < ALL (SELECT salary FROM employees WHERE department_id = 70);
